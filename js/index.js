$(function(){
	...
	$(document).scroll(function(){
				...
				var s = $(document).scrollTop();
					...
					// 判断滚动条大于距离显示
				if(s>=900){
					$('.search_nv').stop().animate({'height':48+'px'},400);
				}else{
					// 判断滚动条小于距离隐藏
					$('.search_nv').stop().animate({'height':'0px'},400);
				}
					// 当滚动距离大于200 点击底部回到顶部
					if(s>=200){
	...
	// 京东上面效果
		$('#gb').click(function(){
			$(this).parents('.banner_top').animate({'height':'0px'},300)
		})
	// 右边固定导航
	$('.rl_xtb_li_wz').hover(function(){
		$(this).siblings('span').stop().animate({'left':-70+'px'},400);
	},function(){
		$(this).siblings('span').stop().animate({'left':'0px'},400);
	})
	
	
	var djtwo=0;
	$('.ms_center .right3').click(function(){
		// 定义全局变量++
				djtwo++;
				if(djtwo>3){
					$(this).siblings('.ms_centerli').css('left','0px');
				djtwo=1;
				}
			$(this).siblings('.ms_centerli').animate({'left':-djtwo*795+'px'},600)
			})
	$('.ms_center .left3').click(function(){
		// 定义全局变量++
				djtwo--;
				
				if(djtwo<0){
					$(this).siblings('.ms_centerli').css('left','-2400px');
				djtwo=2;
				}
			$(this).siblings('.ms_centerli').animate({'left':-djtwo*795+'px'},600)
			})
			// 设置排行榜鼠标滑过效果
			$('.bnlims_right_ul .ms_right_li').mouseenter(function(){
				var jsthis = $(this).index();
				$(this).parents('.bnlims_right_ul').siblings('.bn_li_jh').eq(jsthis).animate({'left':-jsthis*380+'px'})
				})
